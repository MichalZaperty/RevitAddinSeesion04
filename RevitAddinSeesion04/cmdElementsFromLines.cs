                ...
                try
                {
                    foreach (Element e in picklist)
                    {
                        if (e is CurveElement)
                        {
                            ...
                            {
                                case "A-GLAZ":
                                  Wall newGWall = Wall.Create(doc, curve.GeometryCurve, wallGlaz.Id, level.Id, 10, 0, false, false);
                                  Debug.WriteLine(curveGS.Name.ToString());
                                  break;

                                case "A-WALL":
                                    Wall newAWall = Wall.Create(doc, curve.GeometryCurve, wallArch.Id, level.Id, 10, 0, false, false);
                                    Debug.WriteLine(curveGS.Name.ToString());
                                    break;
                                ...
                                case "M-DUCT":
                                    ...
                                    Duct newDuct = Duct.Create(doc, ductSystem.Id, ductType.Id, level.Id, startPoint, endPoint);
                                    Debug.WriteLine(curveGS.Name.ToString());
                                ...
                                case "P-PIPE":
                                    ...
                                    Pipe newPipe = Pipe.Create(doc, pipeSystem.Id, pipeType.Id, level.Id, startPPoint, endPPoint);
                                    Debug.WriteLine(curveGS.Name.ToString());
                                    break;
                            }
                        }
